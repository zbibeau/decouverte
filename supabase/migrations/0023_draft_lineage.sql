-- =====================================================================
-- 0023 — Track lineage from cloned-to-draft rows back to their source.
-- Needed to translate a published chapter/block id to its draft twin
-- when a mutating server action fires against a published-era id.
-- =====================================================================

alter table public.chapter
  add column if not exists cloned_from_chapter_id uuid
    references public.chapter(id) on delete set null;

alter table public.block
  add column if not exists cloned_from_block_id uuid
    references public.block(id) on delete set null;

create index if not exists chapter_cloned_from_idx on public.chapter(cloned_from_chapter_id);
create index if not exists block_cloned_from_idx   on public.block(cloned_from_block_id);

-- Redefine clone_version_as_draft so new rows carry their lineage pointer.
...
begin
  ...
  -- Pass 1: clone chapters with lineage.
  for src_chapter in
    select * from public.chapter where version_id = src_version_id
  loop
    v_old_id := src_chapter.id;
    insert into public.chapter (
      version_id, slug, title, "order", branching_next, wrapper_class, cloned_from_chapter_id
    ) values (
      v_new_version_id,
      src_chapter.slug,
      src_chapter.title,
